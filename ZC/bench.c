...
char* csSlice(char* self, int begin, int end) {
    char* str = (char*)malloc(end - begin);
    memcpy(str, self+begin, end-begin);
    return str;
}
...
static int client_parse(struct client *c, const char *buf, unsigned int len)
{
    ...
    apiName = csSlice(buf, temp1+1, temp2);
    ...
    if (csIsEqual(apiName, "/login")) {
        int fd = open("/Users/zhangxinwei/zc/login.html", O_RDONLY);
        struct stat st;
        stat("/Users/zhangxinwei/zc/login.html", &st);
    ...
    if (csIsEqual(apiName, "/")) {
        
        
        return 0;
    }
    
    if (csIsEqual(apiName, "/addContent")) {
